...
import com.brianna.catalogo_ventas.v1.Modelo.dao.EncargadoInventarioDAO;
...
public class EncargadoInventarioDAOImpl implements EncargadoInventarioDAO{

  
    private final Connection conn = BD.getInstance().getConnection();

    @Override
    public EncargadoInventario getUserByUsernameAndPassword(String usuario, String contraseña) {
        EncargadoInventario encargadoInventario = null;
        String query = "SELECT * FROM encargado_inventario e INNER JOIN usuario u ON e.id_usuario = u.id INNER JOIN persona p ON u.id_persona = p.id WHERE u.usuario = ? AND u.contraseña = ?";
        try (PreparedStatement ps = conn.prepareStatement(query)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {  // El ResultSet también se cierra automáticamente
                if (rs.next()) {
                    ...
                    encargadoInventario = new EncargadoInventario();
                   // cliente.setIdUsuario(rs.getLong("id"));
                    encargadoInventario.setNombre(rs.getString("nombre"));
                    encargadoInventario.setApellido(rs.getString("apellido"));
                    encargadoInventario.setFechaNacimiento(rs.getDate("fecha_nacimiento"));
                    encargadoInventario.setNumero(rs.getString("numero"));
                    encargadoInventario.setDni(rs.getString("dni"));
                    encargadoInventario.setRuc(rs.getString("ruc"));
                    encargadoInventario.setDistrito(rs.getString("distrito"));
                    encargadoInventario.setDireccion(rs.getString("direccion"));
                    encargadoInventario.setUsuario(rs.getString("usuario"));
                    encargadoInventario.setContraseña(rs.getString("contraseña"));
                    encargadoInventario.setCorreo(rs.getString("correo"));
                    encargadoInventario.setRol(rs.getString("rol"));
                }
            }
    ...
    @Override
    public EncargadoInventario selectByID(Long id) {
    ...
    @Override
    public List<EncargadoInventario> selectALL() {
    ...
    @Override
    public EncargadoInventario getUserByUsername(String username) {
       /* Cliente cliente = null;
    ...
    @Override
    public void insert(EncargadoInventario encargadoInventario) {
    ...
    public void deleteUserByUsernameAndPassword(String usuario, String contraseña) {
        String query = "DELETE FROM encargado_inventario WHERE usuario = ? AND contraseña = ?";
    ...
    @Override
    public List<EncargadoInventario> selectByAnyAtributte(String atributte) {
       /* List<Cliente> clientes = new ArrayList<>();
        String query = """
        SELECT * 
        FROM usuario
